...
func (r *AirdropRepo) FindByAccount(accountID, userID uint) ([]model.Airdrop, error) {
	var airdrops []model.Airdrop
...
func (r *AirdropRepo) Delete(id, userID uint) error {
	// Delete tasks first
	r.DB.Where("airdrop_id = ?", id).Delete(&model.Task{})
	return r.DB.Where("user_id = ? AND id = ?", userID, id).Delete(&model.Airdrop{}).Error
}
